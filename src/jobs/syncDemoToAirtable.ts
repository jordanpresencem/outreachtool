import { scrapeFromHtml as parseOnlyfinderHtml } from '../scrapers/onlyfinder';
...
const profiles = parseOnlyfinderHtml(SAMPLE_HTML);

profiles.forEach((profile) => {
  const payload = {
    ONLYFINDER_RID: profile.onlyfinderRid ?? null,
    ONLYFINDER_USERNAME: profile.onlyfinderUsername ?? null,
    ONLYFINDER_PROFILE_URL: profile.onlyfinderProfileUrl ?? null,
    ONLYFANS_USERNAME: profile.onlyfansUsername ?? null,
    ONLYFANS_URL: profile.onlyfansUrl ?? null,
  ...
  const ridLabel = profile.onlyfinderRid ?? 'unknown';
  console.log(`DEMO: would sync profile RID ${ridLabel} with payload:`);
